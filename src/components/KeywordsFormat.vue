<template>
    <section class="my-8">
        <div class="grid grid-cols-2 gap-10">
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <a-button type="primary"
                        @click="onRemoveDuplicate()">Remove duplicate</a-button>
                </div>
                <a-textarea v-model:value="wordStr"
            ...
            <div class="space-y-4">
                <h3 class="font-bold leading-8"> Result: </h3>
                <template v-if="keywords.length">
                    <div class="flex gap-2 flex-wrap">
                        <a-tag v-for="(word, index) of keywords"
                            closable
                            @close.prevent="onRemoveKeyword(index)"
                            class="mr-0"> {{ word }} </a-tag>
                    </div>
                    <div class="italic leading-5">
                        <p> {{ keywordStr }}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <a class="text-blue-500 inline-block cursor-pointer"
                            @click="onCopy">{{ copyText }}</a>
